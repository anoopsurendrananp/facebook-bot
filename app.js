    ...
    data.entry.forEach(function (pageEntry) {
      ...
      pageEntry.messaging.forEach(function (messagingEvent) {
        if (messagingEvent.optin) {
          ...
          /*redis_client.exists(config.get('appSecret').concat(senderInfo), function (err, reply) {
              if (!err) {
                  if (reply === 1) {
                    console.log("Key exists");
                  } else {
                    console.log("Does't exists");
                    redis_client.set(config.get('appSecret').concat(senderInfo), JSON.stringify({context, prev_context, legit_query }));
                  } */
...
function callWatson(user_query, senderInfo, isWelcomeMsg) {
    ...
    /*redis_client.get(config.get('appSecret').concat(senderInfo), function (err, reply) {
      if (err) {
        console.log(err);
        return;
      }
      ...
      assistant.message({
        ...
        workspace_id: config.get('watsonWorkspace'),
        context: context
        ...
        if (err) {
          ...
          redis_client.set(config.get('appSecret').concat(senderInfo), JSON.stringify(sender));
          let messageData = {};
